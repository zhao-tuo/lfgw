...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/project-info'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ProjectInfo) {
        return item.id;
    }
    registerChangeInProjectInfos() {
        this.eventSubscriber = this.eventManager.subscribe('projectInfoListModification', (response) => this.loadAll());
    }
